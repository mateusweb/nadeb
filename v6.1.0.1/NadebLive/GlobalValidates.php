...
class GlobalValidates
{
	/**
	 * Verifica se um e-mail Ž v‡lido
	 * 
	 * @param string $email
	 * @return boolean
	 */
	...
	public static function isCPF($cpf)
	{
		if( preg_match("/^(\d{3}\.){2}\d{3}-\d{2}$/",$cpf) || preg_match("/\d{11}$/",$cpf) )
		{
			...
			else
			{
				...
				if( strcmp( $cpf, $cpf_final ) )	return false;
			}	  
		}
		else
		{
	...
	{
		if (strlen($_value) <> 14) return false;
		...
		if ($_value[12] == $d1 && $_value[13] == $d2) return true;
