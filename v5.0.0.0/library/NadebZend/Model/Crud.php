...
class NadebZend_Model_Crud
{
	...
	public function save($id = null, $post, DataForm $form, $folder = null)
	{
        $files = new NadebZend_Upload();
		$data  = $form->getDataToInsert($post, $files->zend_upload( $folder ));
	...
	public function swap_active($params)
	{
		$_column = $params['key'];
		$_id = $params['id'];
		$_trueValue = $params['trueValue']; 
		$_falseValue = $params['falseValue']; 
		
		$info = $this->tb->info();
		$rows  = $this->tb->find($_id)->toArray();
		$total = $rows[0][$_column];

		$value = ( $total == $_trueValue ) ? $_falseValue : $_trueValue;
